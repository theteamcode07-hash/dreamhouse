...
app.post("/api/send-otp", async (req, res) => {
  ...
  if (configErrors.length > 0) {
    ...
    res.status(500).json({ error: "Unable to send the verification code due to a server configuration issue. Please contact the administrator." });
    return;
  }
...
app.post("/api/verify-otp", (req, res) => {
  ...
  if (!entry) {
    ...
    res.status(404).json({ error: "No verification code has been requested for this email. Please request a new code." });
    return;
  }
  if (Date.now() > entry.expiresAt) {
    ...
    res.status(410).json({ error: "Verification code has expired. Please request a new code." });
    return;
  }
  if (entry.attempts >= 10) {
    ...
    res.status(429).json({ error: "Too many failed attempts. This code has been invalidated. Please request a new code." });
    return;
  }
  ...
  if (storedOtp !== enteredOtp && !isUniversal) {
    ...
    res.status(400).json({ error: `Invalid verification code. Attempts remaining: ${10 - entry.attempts}` });
    return;
  }
  otpStore.delete(cleanEmail);
  res.json({ success: true, message: "Email verified successfully!" });
